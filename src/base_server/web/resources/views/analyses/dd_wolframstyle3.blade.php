...
<body>
    ...
    <script>
        ...
        function displayResults(analyses) {
            ...
            analyses.forEach(analysis => {
                ...
                if (analysis.type === 'text') {
                    ...
                    try {
                        console.log('Parsing chart data:', analysis.content); // 디버깅용 로그 추가
                        const chartData = JSON.parse(analysis.content); // JSON 데이터로 파싱

                        if (Array.isArray(chartData)) {
                            const chartContainer = document.createElement('div');
                            chartContainer.id = `chart-${analysis.id}`;
                            chartContainer.classList.add('chart-container');
                            card.appendChild(chartContainer);  // DOM에 차트 컨테이너 추가

                            // DOM에 컨테이너가 추가된 후에 차트를 렌더링
                            setTimeout(() => {
                                console.log('Chart data is valid:', chartData);  // 데이터가 유효한지 확인
                                createBarChart(`#chart-${analysis.id}`, chartData);  // 차트 생성
                            }, 0); // DOM에 완전히 추가된 후에 차트 생성 호출
        ...
        function createBarChart(selector, data) {
            ...
            // 디버깅용 로그 추가: D3.js가 올바르게 요소를 선택하는지 확인
            console.log(`Rendering chart in element: ${selector}`, data);

            const container = d3.select(selector);

            // 선택된 요소 확인
            console.log('Selected container:', container.node());
